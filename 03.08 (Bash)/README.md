...
PSS: `sed` напрямую в скрипте не задействован, т.к. `AWK` лучше справляется с разбором логов. Однако, стоит отметить, что `sed` можно использовать для быстрого парсига одной строки, но для потоковой обработки выбран `AWK`. В рамках задания данный критерий выполнен в виде демонстрации.
...
Письмо с темой "Nginx report" успешно получено. Содержимое содержию всю требуемую статистику:
```
